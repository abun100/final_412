...
void drawTravelers(void)
{
	...
	if (numTravelersDone == numTravelers) {
		cout << "\nAll travelers finished moving" << endl;
		cout << "Reseting Board\n" << endl;

		// join all threads
		join_threads(info, numTravelers);

		//Clear all traveler segments
		travelerList.clear();
		free(info);
		
		//Clear all partitions
		for(unsigned int k = 0; k < partitionList.size(); k++) {
			partitionList[k].blockList.clear();
		}
		partitionList.clear();
		partStatus.clear();
		...
		for(unsigned int k = 0; k < numRows; k++) {
			for(unsigned int j = 0; j < numCols; j++) {
				grid[k][j] == SquareType::FREE_SQUARE;
			}
		}
		// reset the numTravelers back
		numTravelersDone = 0;

		//re-initialize the entire board and travelers

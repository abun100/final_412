...
bool validUserMove(const TravelerSegment& currentSeg, int travelIndex, int dir);
void travelerExists(int travelerIndex);
...
void handleKeyboardEvent(unsigned char c, int x, int y)
{
	...
	switch (c)
	{
		...
		case '9':
			...
			break;
		case 'w':
			info[travelerInControl].userAcess = 1;
			info[travelerInControl].keypress = 2;
			break;
		case 'a':
			info[travelerInControl].userAcess = 1;
			info[travelerInControl].keypress = 1;
			break;
		case 's':
			info[travelerInControl].userAcess = 1;
			info[travelerInControl].keypress = 0;
			break;
		case 'd':
			info[travelerInControl].userAcess = 1;
			info[travelerInControl].keypress = 3;
			break;
		default:
			ok = 1;
			break;
...
void* moveTraveler(ThreadInfo* travelThread) {
    ...
    const int dc[] = {0, -1, 0, 1};
		...
		while(travelThread->keepGoing) {
			...
			if(travelThread->userAcess == 0 && travelThread->index == travelerInControl) {
				continue;
			}
			...
			if(!travelThread->stuck) {
				int dir;
				//When a key is pressed the user with acess will move
				if(travelThread->userAcess == 1) {
					dir = travelThread->keypress;

					// Check if the user inputed valid input, if not try again
					if(!validUserMove(frontSeg, travelThread->index, dir)) {
						continue;
					}
					// If there are no available directions, then we are stuck
					availableDirections = getAvailableDirections(frontSeg, travelThread->index);
					...
					//Reset the acess user acess back to 0, so only one key can be inputted at a time
					travelThread->userAcess = 0;
				}
				else {
					//Other travelers will move automatically
					dir = availableDirections[rand() % availableDirections.size()];
				}

				// Code is the same after this point
				Direction newDir;
				switch (dir)
				{
					case 0:
				...
				if(grid[frontSeg.row + dr[dir]][frontSeg.col + dc[dir]] == SquareType::HORIZONTAL_PARTITION) {
					...
					if (grid[segments[0].row][segments[0].col] == SquareType::EXIT) {
						...
						numTravelersSolved++;
						travelThread->finished = true;
						travelThread->keepGoing = false;
					}

					counters[travelThread->index] += 1;
					// Uncomment if you want to add delay
					if(travelThread->index != travelerInControl) {
...
bool validUserMove(const TravelerSegment& currentSeg, int travelIndex, int dir) {
	...
	// Bounds check
	if (currentSeg.row + dr[dir] < 0 || currentSeg.row + dr[dir] >= numRows ||
		currentSeg.col + dc[dir] < 0 || currentSeg.col + dc[dir] >= numCols) {
		return false;
	}

	// Allow only the movement of free squares, exit, and partitions 
	if (grid[currentSeg.row + dr[dir]][currentSeg.col + dc[dir]] == SquareType::FREE_SQUARE ||
		grid[currentSeg.row + dr[dir]][currentSeg.col + dc[dir]] == SquareType::EXIT ||
		grid[currentSeg.row + dr[dir]][currentSeg.col + dc[dir]] == SquareType::HORIZONTAL_PARTITION ||
		grid[currentSeg.row + dr[dir]][currentSeg.col + dc[dir]] == SquareType::VERTICAL_PARTITION){

		// create a pair of the free square
		std::pair<int, int> nextSquare = {currentSeg.row + dr[dir], currentSeg.col + dc[dir]};
		TravelerSegment temp;
		temp.row = nextSquare.first;
		temp.col = nextSquare.second;
...
void travelerExists(int travelerIndex) {
	if(travelerIndex >= numTravelers) {
		cout << "You choose a traveler that doesn't exit" << endl;
		exit(0);
	}
	if(info[travelerIndex].finished) {
		cout << "You choose a traveler that already reached the exit" << endl;
		exit(0);
	}
	if(!info[travelerIndex].keepGoing) {
		cout << "You choose a traveler that already terminated" << endl;
		exit(0);
	}
}